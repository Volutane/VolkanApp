...
const fetchGamesByDeveloper = async (accessToken, developerName) => {
  const query = `
    fields name, cover.url, rating, release_dates.human, platforms.name, category, version_parent.name;
    where involved_companies.company.name = "${developerName}" & involved_companies.developer = true;
    sort rating desc;
    limit 50;
  `;
...
const DeveloperGamesScreen = () => {
  const router = useRouter();
  const { developerName } = useLocalSearchParams();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const data = await fetchGamesByDeveloper(accessToken, developerName);
        setGames(data);
      } catch (err) {
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={[styles.emptyText, { color: colors.text }]}>No games found for this developer</Text>
            }
          />
        )}
      </SafeAreaView>
...
export default DeveloperGamesScreen; 