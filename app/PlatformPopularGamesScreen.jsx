...
const fetchGamesByPlatform = async (accessToken, platformName) => {
  const query = `
    fields name, cover.url, rating, release_dates.human, platforms.name, category;
    where platforms.name = "${platformName}";
    sort rating desc;
    limit 50;
  `;
...
const PlatformPopularGamesScreen = () => {
  ...
  useEffect(() => {
    const loadGames = async () => {
      try {
        ...
        const data = await fetchGamesByPlatform(accessToken, platform);
        setGames(data);
      } catch (err) {
    ...
    <ImageBackground source={backgroundImage || { uri: 'https://via.placeholder.com/1080x1920' }} style={styles.container} resizeMode="cover">
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { color: colors.text }]}>{platform} Games</Text>
        </View>
...
export default PlatformPopularGamesScreen;