...
const fetchGamesByGenre = async (accessToken, genreName) => {
  const query = `
...
const GenreGamesScreen = () => {
  ...
  const { genreName } = useLocalSearchParams();
  ...
  useEffect(() => {
    const loadGames = async () => {
      try {
        ...
        const data = await fetchGamesByGenre(accessToken, genreName);
        setGames(data);
      } catch (err) {
    ...
    <ImageBackground source={backgroundImage || { uri: 'https://via.placeholder.com/1080x1920' }} style={styles.container} resizeMode="cover">
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { color: colors.text }]}>{genreName} Games</Text>
        </View>
