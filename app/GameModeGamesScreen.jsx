...
const fetchGamesByGameMode = async (accessToken, gameModeName) => {
  const query = `
    fields name, cover.url, rating, release_dates.human, platforms.name, category;
    where game_modes.name = "${gameModeName}";
    sort rating desc;
    limit 50;
  `;
  ...
  useEffect(() => {
    const loadGames = async () => {
      try {
        ...
        const data = await fetchGamesByGameMode(accessToken, gameModeName);
        setGames(data);
      } catch (err) {
    ...
    <ImageBackground source={backgroundImage || { uri: 'https://via.placeholder.com/1080x1920' }} style={styles.container} resizeMode="cover">
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <Text style={[styles.headerTitle, { color: colors.text }]}>{gameModeName} Games</Text>
        </View>
...
export default GameModeGamesScreen; 